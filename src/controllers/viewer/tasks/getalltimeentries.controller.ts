...
export const getAllTimeEntries = async (
  ...
  try {
    logger.info("📊 getAllTimeEntries called (viewer)");
    ...
    // Get all time entries for the viewer user within their organization
    ...
    logger.info(
      `✅ Found ${allEntries.length} time entries for viewer user ${userId}`
    );
