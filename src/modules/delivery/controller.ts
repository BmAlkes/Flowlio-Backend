...
const handler=(action:'list'|'request'|'decide'):RequestHandler=>async(req,res)=>{
 try{
  ...
  const data=action==='list'?await service.list(req.user!,projectId,req.query.page??1):action==='request'?await service.request(req.user!,projectId,req.body):await service.decide(req.user!,projectId,String(req.params.reviewId),req.body);
