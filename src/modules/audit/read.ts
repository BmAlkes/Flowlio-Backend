...
const authorized=`with permitted as (
 ...
 cross join lateral (select coalesce(jsonb_object_agg(d.key,d.value),'{}'::jsonb) changes from jsonb_each(e.changes) d where
  (e.resource_type='project' and (d.key=any(array['start_date','end_date','status','visibility','assigned_to']) or ($3::boolean and d.key='budget')))
 ...
 and ((e.resource_type in ('project','delivery_review') and p.id is not null and (p.created_by=$2 or p.assigned_to=$2 or p.visibility='public'))
