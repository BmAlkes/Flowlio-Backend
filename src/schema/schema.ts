...
export const clientRequests=pgTable("client_requests",{
 ...
 state:text("state").notNull().default("open"),revision:integer("revision").notNull().default(0),cycle:integer("cycle").notNull().default(1),reminderHours:integer("reminder_hours").notNull().default(0),reminderChannel:text("reminder_channel").notNull().default("internal"),reminderNextAt:timestamp("reminder_next_at",{withTimezone:true}),reminderCount:integer("reminder_count").notNull().default(0),reason:text("reason"),
...
},t=>({project:index("client_requests_project_idx").on(t.organizationId,t.projectId,t.createdAt,t.id),reminder:index("client_requests_reminder_idx").on(t.reminderNextAt).where(sql`${t.state}='open' and ${t.reminderHours}>0`),kind:check("client_requests_kind_check",sql`${t.kind} in ('briefing','question','file')`),state:check("client_requests_state_check",sql`${t.state} in ('open','answered','completed','cancelled')`),hours:check("client_requests_reminder_hours_check",sql`${t.reminderHours} in (0,24,48,72,168)`),channel:check("client_requests_reminder_channel_check",sql`${t.reminderChannel} in ('internal','email','push')`),count:check("client_requests_reminder_count_check",sql`${t.reminderCount} between 0 and 3`)}));
export const clientRequestResponses=pgTable("client_request_responses",{id:text("id").primaryKey(),requestId:text("request_id").notNull().references(()=>clientRequests.id,{onDelete:"cascade"}),cycle:integer("cycle").notNull(),actorId:text("actor_id").notNull(),message:text("message").notNull(),answers:jsonb("answers").notNull(),attachments:jsonb("attachments").notNull(),createdAt:timestamp("created_at",{withTimezone:true}).notNull().defaultNow()},t=>({cycle:unique("client_request_response_cycle_key").on(t.requestId,t.cycle)}));
export const clientRequestCommands=pgTable("client_request_commands",{id:text("id").primaryKey(),requestId:text("request_id").notNull().references(()=>clientRequests.id,{onDelete:"cascade"}),actorId:text("actor_id").notNull(),fingerprint:text("fingerprint").notNull(),createdAt:timestamp("created_at",{withTimezone:true}).notNull().defaultNow()});
export const clientRequestReminders=pgTable("client_request_reminders",{id:text("id").primaryKey(),requestId:text("request_id").notNull().references(()=>clientRequests.id,{onDelete:"cascade"}),cycle:integer("cycle").notNull(),sequence:integer("sequence").notNull(),jobId:text("job_id"),outcome:text("outcome").notNull().default("queued"),createdAt:timestamp("created_at",{withTimezone:true}).notNull().defaultNow()},t=>({sequence:unique("client_request_reminder_key").on(t.requestId,t.cycle,t.sequence)}));
export const clientRequestEvents=pgTable("client_request_events",{id:text("id").primaryKey(),requestId:text("request_id").notNull().references(()=>clientRequests.id,{onDelete:"cascade"}),action:text("action").notNull(),actorId:text("actor_id").notNull(),reason:text("reason").notNull(),createdAt:timestamp("created_at",{withTimezone:true}).notNull().defaultNow()},t=>({request:index("client_request_events_request_idx").on(t.requestId,t.createdAt,t.id)}));

export const capacityAbsences=pgTable('capacity_absences',{
 id:text('id').primaryKey(),organizationId:text('organization_id').notNull().references(()=>organizations.id,{onDelete:'cascade'}),userId:text('user_id').notNull().references(()=>users.id,{onDelete:'cascade'}),startDate:text('start_date').notNull(),endDate:text('end_date').notNull(),kind:text('kind').notNull(),revision:integer('revision').notNull().default(0),createdBy:text('created_by').notNull(),createdAt:timestamp('created_at',{withTimezone:true}).notNull().defaultNow(),updatedAt:timestamp('updated_at',{withTimezone:true}).notNull().defaultNow()
},t=>({dates:check('capacity_absence_dates',sql`${t.startDate} <= ${t.endDate}`),kind:check('capacity_absences_kind_check',sql`${t.kind} in ('vacation','unavailable')`),lookup:index('capacity_absences_org_user_idx').on(t.organizationId,t.userId,t.startDate,t.endDate)}));
export const capacityScenarios=pgTable('capacity_scenarios',{
 id:text('id').primaryKey(),organizationId:text('organization_id').notNull().references(()=>organizations.id,{onDelete:'cascade'}),createdBy:text('created_by').notNull().references(()=>users.id,{onDelete:'cascade'}),title:text('title').notNull(),weekStart:text('week_start').notNull(),weeks:integer('weeks').notNull(),base:jsonb('base').notNull(),baseHash:text('base_hash').notNull(),changes:jsonb('changes').notNull().default(sql`'[]'::jsonb`),revision:integer('revision').notNull().default(0),state:text('state').notNull().default('draft'),creationHash:text('creation_hash').notNull(),applyKey:text('apply_key'),applyHash:text('apply_hash'),appliedIds:jsonb('applied_ids').notNull().default(sql`'[]'::jsonb`),createdAt:timestamp('created_at',{withTimezone:true}).notNull().defaultNow(),updatedAt:timestamp('updated_at',{withTimezone:true}).notNull().defaultNow(),appliedAt:timestamp('applied_at',{withTimezone:true})
},t=>({weeks:check('capacity_scenarios_weeks_check',sql`${t.weeks} between 1 and 12`),state:check('capacity_scenarios_state_check',sql`${t.state} in ('draft','applied')`),author:index('capacity_scenarios_author_idx').on(t.organizationId,t.createdBy,t.createdAt)}));
