...
if(!process.env.RELEASE_TEST_DATABASE_URL){test('Release PostgreSQL integration (set RELEASE_TEST_DATABASE_URL)',{skip:true},()=>{});}
 ...
 const addMigration=async(temp,sql)=>{
  ...
  journal.entries.push({idx:2,version:'7',when:journal.entries[1].when+1,tag:'0002_test',breakpoints:true});
  await fs.writeFile(file,JSON.stringify(journal));await fs.writeFile(path.join(temp,'0002_test.sql'),sql);
 };
 ...
 test('empty database becomes complete, with billing and timer guards; repeated startup is read-only',async()=>{
  await runReleaseMigrations(pool);const first=await history();assert.equal(first.length,2);
  const tables=(await pool.query("select count(*) from pg_tables where schemaname='public'")).rows[0].count;assert.equal(Number(tables),61);
  const triggers=(await pool.query("select tgname from pg_trigger where not tgisinternal order by tgname")).rows.map(r=>r.tgname);
  assert.deepEqual(triggers,['invoices_assign_number','time_entries_guard_active','time_entries_protect_billing']);
 ...
 test('five instances starting together apply each migration once',async()=>{
  await Promise.all(Array.from({length:5},()=>runReleaseMigrations(pool)));assert.equal((await history()).length,2);
 });
 ...
 test('failed migration rolls back its DDL and history, leaving a retry possible',async()=>{
  ...
  await assert.rejects(runReleaseMigrations(pool,temp));assert.equal((await history()).length,2);
  ...
  await fs.writeFile(path.join(temp,'0002_test.sql'),'alter table users add column rollback_probe text;');
  await runReleaseMigrations(pool,temp);assert.equal((await history()).length,3);
 });
 ...
 test('CLI exits nonzero on SQL failure and zero only on successful completion',async()=>{
  ...
  await fs.writeFile(path.join(temp,'drizzle/releases/0002_test.sql'),'select 1;');
  assert.equal(await cli(),0);assert.equal((await history()).length,3);
 });
}
